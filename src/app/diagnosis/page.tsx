 ...
 * 무료 병원 진단.
 *
 * 전 사이트의 CTA가 여기로 모입니다. 그래서 이 페이지는 브로슈어가 아니라
 * **폼이 첫 화면**입니다 — 도착하자마자 1번 문항이 보이고, 왜 진단부터인지·
 * 무엇을 보는지는 그 아래에서 설명합니다. 누른 사람이 이미 하기로 한 일을
 * 네 섹션 뒤로 미루지 않습니다.
 */

/** 폼 옆에 두는 세 가지 사실 — 시작을 망설이게 하는 질문에 미리 답합니다 */
const FACTS = [
  { k: "문항", v: `${DIAGNOSIS_QUESTIONS.length}개 · 약 3분` },
  { k: "비용", v: "없음" },
  { k: "회신", v: "담당 플래너가 직접 읽고 영업일 1일 내" },
];
...
export default function Page() {
  return (
    <>
      {/* ── 첫 화면: 제목 + 폼. 사진 히어로 없음 ── */}
      <section className="diagnosis-top">
        <Container>
          <div className="diagnosis-top-grid">
            <div className="diagnosis-intro" data-reveal>
              <p className="label label-ko">{DIAGNOSIS.sub}</p>
              ...
              <p className="prose-ko diagnosis-lead">{DIAGNOSIS.body}</p>
              ...
              <p className="diagnosis-note">
                ...
                보장하지 않습니다. 입력하신 정보는 상담 목적 외에 사용하지 않습니다.
              </p>
            </div>
      ...
      <Section no="03" label="시작 전에" tone="paper">
        ...
        <div className="mt-12">
          <a href="#form" className="editorial-link">
            위로 올라가 진단 시작하기 <span aria-hidden="true">→</span>
          </a>
        </div>
      </Section>
    </>
